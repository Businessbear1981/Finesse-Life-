...
export default function VaultEntrance() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("finesse_gender");
    if (saved) {
      // Already selected in a previous session — skip the selector
    }
  }, []);

  const selectGender = (gender: "feminine" | "masculine") => {
    localStorage.setItem("finesse_gender", gender);
    // New users go to onboarding first; both genders hit same onboarding — content shifts based on lens
    ...
    if (val.length === 6) {
      setTimeout(() => {
        if (val === DEMO_PIN) {
          ...
          // Always show gender selector — user can switch sides each session
  ...
  }, [opening, error, showGenderSelect, router, shakeControls, reset]);
  ...
  return (
    ...
    >
      ...
      />
      <div className="film-grain pointer-events-none" style={{ opacity: 0.15 }} />
      ...
      {["left-[6%]", "right-[6%]"].map((pos, i) => (
        <div key={pos} className={`pointer-events-none absolute ${pos} top-1/2 -translate-y-1/2`}>
          <div className="flex flex-col items-center">
            ...
            <div className="w-32 h-32 -mt-1 animate-chandelier-pulse"
              style={{ background: "radial-gradient(circle, rgba(255,169,107,0.2) 0%, transparent 65%)", animationDelay: `${i * 2}s` }}
            />
          </div>
        </div>
      ))}
      ...
      <motion.div animate={shakeControls} className="relative z-10 w-[340px] h-[340px] max-w-[88vw] max-h-[88vw] mx-auto">
        ...
        {/* ── INNER GEAR RING — rotates on digit 1 ── */}
        <motion.div
        ...
        {/* ── SIX PETALS — fold back one by one ── */}
          ...
          return (
            <motion.div
              ...
              animate={{
                scale: folded ? 0 : 1,
                opacity: folded ? 0 : 1,
                rotate: folded ? (angle + 90 + 180) : (angle + 90),
              }}
            ...
            >
              ...
              <div className="relative w-[44px] h-[52px]"
                style={{
                  ...
                  background: "linear-gradient(to bottom, #F0D88A, #C9A961, #6B5028)",
                }}
              >
        ...
        {/* ── TUMBLER BAR — slides right on digit 3 ── */}
        <motion.div
          ...
          style={{
            ...
            boxShadow: "0 2px 4px rgba(0,0,0,0.4), inset 0 1px 0 rgba(232,200,122,0.3)",
          }}
        ...
        >
          ...
          <div className="flex justify-evenly h-full items-center px-2">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={`tm${i}`} className="w-px h-[5px] bg-ink/30" />
            ))}
          </div>
        </motion.div>

        {/* ── VERTICAL BOLTS — retract on digit 5 ── */}
        {[
        ...
        {/* ── CENTRAL WHEEL — ornate dial, rotates on 2/4/6 ── */}
        <motion.div
          ...
          style={{
            ...
            boxShadow: "0 0 20px rgba(201,169,97,0.15), inset 0 0 15px rgba(201,169,97,0.08)",
          }}
          animate={{
            rotate: (count >= 2 ? -30 : 0) + (count >= 4 ? -30 : 0) + (count >= 6 ? -30 : 0),
          }}
        ...
        >
          ...
          {/* Center diamond */}
          <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-4 h-4 rotate-45 border-2 border-brass/50"
            style={{ background: "linear-gradient(135deg, #F0D88A, #C9A961)" }}
          />
        ...
        {/* ── DOOR OPEN — left and right halves swing ── */}
        {opening && (
          <>
            <motion.div
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1 }} className="mt-6 relative z-10">
              ...
              >
                ...
                {filled ? (
                  <motion.span
                    ...
                    style={{ color: "#E8C87A", textShadow: "0 0 10px rgba(232,200,122,0.3)" }}
        ...
        {error && (
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="text-center mt-4 font-body text-sm italic" style={{ color: "#F4E0A0", textShadow: "0 0 8px rgba(244,224,160,0.2)" }}
          >Try again</motion.p>
        )}
      </motion.div>
      ...
      {!showGenderSelect && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.2 }} className="mt-8 relative z-10 text-center">
          <p className="font-body text-xs text-cream/15 italic">
            Not a member?{" "}
            <span className="text-brass/30 hover:text-brass cursor-pointer transition-colors">Request an invitation</span>
          </p>
        </motion.div>
      )}

      {/* ═══ GENDER SELECTOR — appears after vault opens ═══ */}
      <AnimatePresence>
        {showGenderSelect && (
          <motion.div
